# MD-102 Expert Lab Guide: Enterprise Edition

**Project:** Microsoft 365 Endpoint Administrator Master Lab & Blueprint  
**Platform:** Microsoft Intune, Microsoft Entra ID, Microsoft Defender for Endpoint, Windows 11 Enterprise  
**Subscription:** Microsoft 365 E5 (Tenant Total: 25 Licenses | Active Allocated Budget: 20 Licenses | Safety Reserve: 5 Licenses)  
**Owner:** Özen Ucan  
...
This lab guide is engineered with a **Zero-Compromise, Production-First Architecture**. It serves not only as a comprehensive syllabus for **Exam MD-102: Managing and Securing Microsoft 365 Endpoints by using Intune**, but as an enterprise-grade standard operating procedure (SOP) for senior endpoint architects.
...
| **Corporate Persona** | Adele Vance (IT) | `adele.vance@<tenant>` | Windows 11 Corp PC (VM 1); Pilot update ring; EPM user | **1** |
| **Corporate Persona** | Alex Wilber (Finance) | `alex.wilber@<tenant>` | Windows 11 Corp PC (VM 2); BitLocker escrow; LAPS; Custom Compliance | **1** |
| **Corporate Persona** | Megan Bowen (HR) | `megan.bowen@<tenant>` | Windows 11 Autopilot PC (VM 3); Autopilot v2 Device Prep; M365 Apps | **1** |
| **BYOD Persona** | Joni Sherman (Sales) | `joni.sherman@<tenant>` | BYOD Windows / Android; MAM-WE; App Protection Policies | **1** |
| **Mobile Persona** | Diego Siciliani (Field) | `diego.s@<tenant>` | Android Enterprise Work Profile / Dedicated Kiosk (AVD Emulator) | **1** |
| **Executive Persona** | Miriam Graham (Exec) | `miriam.g@<tenant>` | macOS / iOS; Platform SSO; FileVault key escrow | **1** |
| **Specialized Test** | Intune Helpdesk Operator | `intune-operator@<tenant>` | Restricted RBAC Helpdesk role; Scope Tags testing (`Tag-Finance`) | **1** |
| **Specialized Test** | Security Operator | `security-operator@<tenant>` | Restricted Defender MDE incident responder | **1** |
| **Specialized Test** | Shared Kiosk Account | `kiosk-user@<tenant>` | Multi-user shared device & Assigned Access testing | **1** |
| **Specialized Test** | Pilot User 01 | `pilot.user01@<tenant>` | Pre-production testing for Win32 apps & Feature Updates | **1** |
| **Specialized Test** | Pilot User 02 | `pilot.user02@<tenant>` | Quality update expedited zero-day testing | **1** |
| **Staging Buffer** | Dynamic Test Users (1–6)| `staging.user01-06@<tenant>` | Temporary accounts used during multi-device provisioning | **6** |
...
- [ ] Configure Scoped Intune Role Assignment:
  - Assign built-in role **Help Desk Operator** to `intune-operator@<tenant>`.
...
- [ ] **Validation:** Sign in as `intune-operator@<tenant>` and verify that non-Finance devices and IT policies are completely hidden from view.
...
2. Sign in to the workstation with licensed user `alex.wilber@<tenant>`.
...
- [ ] Boot VM 2, verify Contoso branded login screen appears, sign in with `alex.wilber@<tenant>`.
...
4. On a clean un-hashed VM (VM 3), boot to OOBE and sign in with `megan.bowen@<tenant>`. Verify seamless automated provisioning.
...
- **Users:** Include: *All Users*. Exclude: `admin-global-emergency@<tenant>` & `GRP-USR-EXCLUDE-CA`.
...
- [ ] **Selective Wipe Test**:
  ...
  - Select user `joni.sherman@<tenant>` > Verify corporate data is erased while personal photos/data remain untouched.
...
- [ ] Configuration Keys:
  - `HomepageLocation` = `https://contoso.sharepoint.com`
  - `NewTabPageLocation` = `https://contoso.sharepoint.com`
...
```text
Ring 0 (Canary/IT): 0-day deferral, immediate pilot (Adele Vance - VM 1)
Ring 1 (Pilot Users): 3-day quality deferral, 7-day feature deferral (Megan Bowen - VM 2)
Ring 2 (Broad Production): 7-day quality deferral, 30-day feature deferral (Alex Wilber - VM 3)
```
...
Choose either the **10-Day Accelerated Sprint** (Full-time intensive: ~2–3 labs / 2.5–3.5 hours per day) or the **4-Week Modular Track** (Part-time: ~5–6 hours per week).
...
## Scenario: Contoso Modern Endpoint Modernization

Contoso Endpoint Lab is modernizing its endpoint estate for **20 active users/devices** across IT, Finance, HR, Sales, and Field Operations within the 20-seat allocated Microsoft 365 E5 license budget (keeping a 5-seat safety reserve).
