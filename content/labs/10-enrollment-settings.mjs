export default {
      ...
      tasks: [
        ...
        },
        {
          ...
          steps: [
            ...
            {
              text: "In the left navigation menu under **Devices**, expand **Manage devices**, select **Device categories**, then select **Create**:",
              ...
              parts: [
                ...
                },
                {
                  kind: "substeps",
                  items: [
                    { text: "On the **Basics** tab, enter the **Name** and **Description** from above, then select **Next**." },
                    { text: "On the **Scope tags** tab, leave **Default** assigned, then select **Next**." },
                ...
                },
                {
                  kind: "callout",
                  variant: "important",
                  text: "Device categories do **not** prompt Windows users during enrollment. The interactive Company Portal selection prompt is exclusive to iOS/iPadOS, macOS, and Android during Company Portal enrollment. On Windows, categories must be manually assigned by an administrator in Intune under **Devices** > **All devices** > select device > **Properties**, or self-selected by the user via the Company Portal web portal at `portal.manage.microsoft.com` under **Devices** > **Category**. Do not expect `MD102-VM2-Alex` to prompt for a category when enrolled in Exercise 3."
                },
                {
                  kind: "callout",
                  variant: "tip",
                  text: "When device categories exist, mobile users are prompted to pick one during enrollment, and the choice can drive dynamic group membership through `device.deviceCategory`. It is an effective way to let users self-classify hardware you cannot pre-register."
                }
              ]
            }
          ],
          result: {
            text: "You know where device categories and enrollment notifications live, and how categories behave across different operating systems.",
            verify: [
              { text: "The **Shared workstation** category appears under **Devices** > **Manage devices** > **Device categories**." }
            ]
          }
        }
      ]
      ...
      tasks: [
        {
          ...
          steps: [
              ...
              parts: [
                {
                  ...
                  items: [
                    ...
                    { text: "Alternatively, trigger enrollment from an elevated prompt with `DeviceEnroller.exe /c /AutoEnrollMDM`." }
                  ]
                }
              ]
            },
            {
              text: "Confirm enrollment from the client (run in PowerShell on **MD102-VM2-Alex**):",
              parts: [
                {
                  kind: "code",
                  lang: "powershell",
            ...
            {
              text: "Check the management registry keys where enrollment records itself (run in an elevated Administrator PowerShell session on **MD102-VM2-Alex**):",
              parts: [
                {
                  kind: "code",
                  lang: "powershell",
              ...
              parts: [
                {
                  kind: "callout",
                  variant: "tip",
                  text: "The device appears in Intune under its Windows hostname (e.g. `DESKTOP-XXXXXXX`), not the Hyper-V VM name `MD102-VM2-Alex`. You can identify it by looking for **Alex Wilber** in the **Primary user** column or checking the **Management name**."
                },
                {
                  kind: "verify",
                  text: "The device (listed under its Windows hostname, e.g. `DESKTOP-...`) is listed with **Managed by** of **Intune** and an **Ownership** of **Personal** — ownership is corrected in lab 11."
                }
              ]
            },
            {
              text: "Lab 7 left one check unfinished for want of a device. Finish it now: open a private window, sign in as `helpdesk.operator@<tenant>.onmicrosoft.com`, then open **Devices** > **All devices** and select the device (e.g. `DESKTOP-XXXXXXX`).",
              nav: ["Devices", "All devices"],
              parts: [
                {
                  kind: "callout",
                  variant: "important",
                  text: "If the device does not appear for `helpdesk.operator`: verify that the **Help Desk — all devices** role assignment from lab 7 has the **Default** scope tag assigned (**Tenant administration > Roles > All roles > Help Desk Operator > Assignments > Properties**). If **Scope tags** shows **No**, the operator cannot see any objects carrying the Default tag. Also, if you already completed lab 8 and scoped the role to `TAG-FINANCE`, ensure the device carries `TAG-FINANCE` under its Properties > Scope tags and has Alex Wilber set as Primary user."
                },
                {
                  kind: "verify",
  ...
  troubleshooting: [
    ...
    },
    {
      ...
      rootCause:
        "The Help Desk Operator role assignment has no scope tags assigned (showing 'No' under Scope tags), which hides all objects carrying the Default scope tag. Alternatively, if lab 8 was completed early, the role may be scoped to TAG-FINANCE while the device only carries Default, or the device lacks an assigned Primary user. Finally, the operator may be scanning for the Hyper-V name 'MD102-VM2-Alex' instead of the Windows computer name ('DESKTOP-XXXXXXX').",
      diagnostic: {
        lang: "text",
        code: "1. In admin session: Tenant administration > Roles > All roles > Help Desk Operator > Assignments > Properties > Scope tags.\n2. In admin session: Devices > All devices > [Device] > Properties > Scope tags and Primary user."
      },
      resolution:
        "Ensure the Help Desk Operator assignment includes the Default scope tag (or add TAG-FINANCE to the device). Confirm Alex Wilber is set as the Primary user on the device, and verify the operator looks for the Windows computer name (e.g. DESKTOP-...) rather than the VM name."
    }
  ],
